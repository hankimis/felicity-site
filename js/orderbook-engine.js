(function(){
  ...
  class BinanceOrderbookAdapter {
    ...
    openStream(symbol, { onMessage, onError, onClose, signal }){
      ...
      ws.onmessage = (ev)=>{
        try{
          ...
          const state = this._normalizeArrays(bids, asks, 50);
    ...
    _normalizeArrays(bids, asks, limit){
      try{
        bids = (bids||[]).filter(x=> isFinite(x.price)&&isFinite(x.size)).sort((a,b)=> b.price - a.price).slice(0, limit);
        asks = (asks||[]).filter(x=> isFinite(x.price)&&isFinite(x.size)).sort((a,b)=> a.price - b.price).slice(0, limit);
        return { bids, asks };
  ...
  class OrderbookService {
    ...
    async _connect(symbol){
      ...
      // 1) snapshot
      try { const snap = await this.adapter.fetchSnapshot(symbol, { signal: ctrl.signal, limit: 50 }); if (ctrl.signal.aborted) return; update(snap); } catch(_){}
      ...
      // 4) periodic REST resync (15s)
      try{ clearInterval(this._restIv); }catch(_){ }
      this._restIv = setInterval(async ()=>{
        if (this._disposed || ctrl.signal.aborted) return;
        try{ const snap = await this.adapter.fetchSnapshot(symbol, { signal: ctrl.signal, limit: 50 }); if (!ctrl.signal.aborted) update(snap); } catch(_){ }
      }, 15000);
    }
