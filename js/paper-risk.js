(function(){
  ...
  class RiskFunding {
    ...
    // 포지션별 안전 마크가격: 현재 심볼 가격이 아닌 해당 포지션 심볼 캐시 우선, 없으면 진입가
    _safeMarkFor(p){
      try{
        const s=this.getters.getState();
        // 외부 엔진에 캐시가 있을 수 있으나 이 모듈은 독립적이므로 state에 심볼별 캐시가 없다면 lastPrice로는 판단하지 않음
        // 보수적으로 진입가 사용
        const entry=Number(p&&p.entry);
        return (isFinite(entry)&&entry>0)? entry : (Number(s&&s.lastPrice)||0);
      }catch(_){ return Number(p&&p.entry)||0; }
    }
    getMaintenanceRate(p){
      ...
      if (brackets && brackets.length){
        ...
        const ref = this._safeMarkFor(p);
        const notional = Math.max(0, ref * p.amount);
    ...
    applyFundingIfDue(){
      ...
      let changed=false;
      for (const p of s.positions){ const mark=this._safeMarkFor(p); const notional=Math.max(0, mark*p.amount); if (notional<=0) continue; const base=notional*Math.abs(rate); const flow=(rate>0)?(p.side==='long'?-base:+base):(p.side==='long'?+base:-base); if (p.mode==='isolated'){ p.margin=Math.max(0,p.margin+flow);} else { s.balanceUSDT+=flow; } changed=true; }
      s.lastFundingTs=now; return changed;
    }

    checkLiquidations(){
      const s=this.getters.getState(); let liq=0; const totalUPnL=s.positions.reduce((a,p)=>a+this.calcPnL(p,this._safeMarkFor(p)),0); const remain=[];
      for (const p of s.positions){ const mark=this._safeMarkFor(p); const notional=Math.max(0, mark*p.amount); const mmr=this.getMaintenanceRate(p); const maint=notional*mmr; const feeEst=notional*FEE_TAKER; const pnl=this.calcPnL(p,mark); let available; if (p.mode==='isolated'){ available=p.margin+pnl-feeEst; } else { available=s.balanceUSDT + totalUPnL - feeEst; } if (available - maint < -1e-8){ const liqFee=Math.max(0, mark*p.amount*FEE_TAKER); s.balanceUSDT += Math.max(0, p.margin + pnl - liqFee); liq++; } else { remain.push(p); } }
    ...
    getTierInfo(p){
      const brackets = (this.getters && this.getters.getBrackets && this.getters.getBrackets()) || null;
      const s=this.getters.getState();
      const notional=Math.max(0,this._safeMarkFor(p)*p.amount);
      if (brackets && brackets.length){
